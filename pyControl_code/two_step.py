...
class Outcome_generator:
    # Determines the second-step state and outcome on each trial and when block 
    # transitions occur.
    ...
    def second_step(self, second_step_state):
        ...
        if self.trans_crit_reached:
            ...
            if (((self.state['trans'] == (self.state['reward'] == 0)) # High is good option..
                and (self.mov_ave.value > self.threshold))              # ..and high treshold crossed.              
            or ((self.state['trans'] != (self.state['reward'] == 0))  # or Low is good option..
                and (self.mov_ave.value < (1. - self.threshold)))):     # .. and low threshold crossed.
...
def wait_for_poke_out(event):
    if event == 'entry':
        if not (hw.left_poke.value() or \
                hw.right_poke.value()):
            goto_state('inter_trial') # Subject already left poke.
