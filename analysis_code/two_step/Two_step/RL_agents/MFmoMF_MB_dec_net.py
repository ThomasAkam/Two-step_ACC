...
class MFmoMF_MB_dec_net(RL_agent):
    '''Mixture agent with forgetting and motor level model free, no elegibility traces.'''

    def __init__(self, kernels = ['bs', 'ck', 'rb']):
        self.name = 'MFmoMF_MB_dec_net'
        self.param_names  = ['alpQ', 'decQ', 'act', 'alpT', 'decT',
                             'G_td', 'G_mb']
        self.param_ranges = ['unit']*5 + ['pos']*2
    ...
    def session_likelihood(self, session, params_T, get_DVs = False):
        ...
        # Unpack parameters.
        alpQ, decQ, act, alpT, decT, G_td, G_mb = params_T[:7]
        ...
        for i, (c, s, o, ps) in enumerate(zip(choices[:-1], second_steps, outcomes, prev_sec_steps)): # loop over trials.
            ...
            Q[c,i+1]    = (1.-alpQ)*Q[c,i]    + alpQ*V[s,i]# First step TD update.
            P[c,ps,i+1] = (1.-alpQ)*P[c,ps,i] + alpQ*V[s,i] # First step TD update.
